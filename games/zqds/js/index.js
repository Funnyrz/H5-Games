var placedata=["农坛","丰体","天安门","天坛","鸟巢","南锣鼓巷","北海","永定门","五道口","西单","宣武门","马甸","雍和宫","颐和园","圆明园"];
var placepos=0;
var Main = {};
...
Main.shareTextMap = {
	...
	,"1": {
		tip: "没吃饱饭吗,用力用力!!"
		,title: "我把足球踢出了${m}米，击败了${n}%人，来挑战我吧！"
	}
	,"2": {
		tip: "哎呦，你真的吊炸天了!"
		,title: "我把足球踢出了${m}米，击败了${n}%人，来挑战我吧！"
...
Main.initGame = function(){
	...
	this.knockPoint = Math.ceil(bear.y + bearBound.height * 0.2);
	
	if(Main.debug){
		Main.log("计算击打点:" + this.knockPoint);
		var shape = new createjs.Shape();
		shape.graphics.beginFill("#ff0000").drawRect(0, 0, 50, 20);
		shape.x = this.width -200;
	...
	var top = new createjs.Text("TOP:", "20px Arial Black", "#336600");
	score.addChild(top);
	var topWidth = top.getMeasuredWidth();
	var text = new createjs.Text("", "20px Arial Black", "#FF0000");
	text.x = topWidth+20;
	...
	var guideText = new createjs.Text("", "24px Arial Black", "#2B5580");
	guideText.y = 25;
...
Main.reset = {
	...
	,penguin: function(){
		...
		penguin.x = Main.width - Main.penguinOffsetX-10;
		penguin.y = 32;
		penguin.alpha = 1;
		penguin.rotation = 0;
...
Main.moveGuide = function(distance, totalDistance){
	...
	if(this.guideMoveLength >= this.guideDistance + this.guideWidth){
		Main.guideMoveIndex++;
		var place=placedata[placepos];
			placepos=(placepos+1)%placedata.length;
		this.reset.guide(place+Main.guideMoveIndex + "00M");
...
Main.on("gameOver", function(event, distance, x){
	...
	try{
		localStorage["zqdsMaxScore"] = Main.maxScore;
	}catch(e){
